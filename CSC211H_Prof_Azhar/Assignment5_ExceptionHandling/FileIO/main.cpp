...
int main(int argc, char **argv)
{
    ...
    // added exception handling for files i/o
    try
    {
        if (!currentScoresFile)
            throw string("File Error: Could not open currenScores.txt");
        if (!commandFiles)
            throw string("File Error: Could not open commands.txt");
        if (!overallGamePlay)
            throw string("File Error: Cout not open overallGamePlayed.txt");
    }
    catch (const string &fileError)
    {
        cout << fileError << endl;
    }
    ...
    catch (int error)
    {
        if (error == 1)
        {
            cout << "Error: Please enter exactly 6 coordinate values." << endl;
            cout << "Example: ./main 1 2 3 4 5 6" << endl;
        }
        else if (error == 2)
        {
        ...
        {
            cout << "Error: Input is not in range [0,9]." << endl;
        }
...
void saveCommands(fstream &file, Robot &robot, int &command)
{
    if (!file)
    {
        // if the file is not open. throw string object
        throw "Warning!Command File is not open. \nDatas will not be saved.\n";
    }
    else
    {
        char c = command + '0';
